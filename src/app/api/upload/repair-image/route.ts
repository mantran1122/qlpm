...
export async function POST(req: NextRequest) {
  ...
  const rl = rateLimit(`repair-upload:${auth.userId}`, 20, 3600)
  ...
  await fs.mkdir(REPAIR_DIR, { recursive: true })
  ...
  for (const file of files) {
    ...
    await fs.writeFile(path.join(REPAIR_DIR, filename), outBuf)
    urls.push(`/api/repair-image/${filename}`)
  }
